...
"""
    ...
    def _extend_user_input(cls, questions):
        questions.generate_cases(
            "preopt",
            {
                key: software.colt_user_input
                for key, software in implemented_qm_software.items()
            },
        )
        questions.generate_cases(
            "software",
    ...
    def scan_software_is_defined(self):
        return self.softwares["scan_software"] is not self.softwares["software"]

    @property
    def charge_software_is_defined(self):
        return self.softwares["charge_software"] is not self.softwares["software"]

    def hessian_name(self, software):
        return self.pathways.hessian_filename(
            software, self.config.charge, self.config.multiplicity
        )

    def grad_name(self, software):
        return self.pathways.grad_filename(
            software, self.config.charge, self.config.multiplicity
        )

    def charge_name(self, software):
        return self.pathways.charge_filename(
            software, self.config.charge, self.config.multiplicity
        )

    def scan_name(self, software):
        return self.pathways.scan_filename(
            software, self.config.charge, self.config.multiplicity
        )

    def scan_sp_name(self, software, i):
        return self.pathways.scan_sp_filename(
            software, self.config.charge, self.config.multiplicity, i
        )

    def sp_name(self, software, i):
        return self.pathways.sp_filename(
            software, self.config.charge, self.config.multiplicity
        )

    def opt_name(self, software):
        return self.pathways.opt_filename(
            software, self.config.charge, self.config.multiplicity
        )

    def Calculation(self, filename, required_files, *, folder=None, software=None):
        return self.job.Calculation(
            filename, required_files, folder=folder, software=software
        )
    ...
    def setup_scan_calculation(
        self, folder, scan_hash, scanned_atomids, coords, atomids
    ):
    ...
    def setup_hessian_calculations(self, parent, iterator):
        ...
        for i, (coords, atnums) in iterator:
            ...
            if not calculation.input_exists():
                with open(calculation.inputfile, "w") as file:
                    self._write_hessian(
                        software, file, calculation.base, coords, atnums
                    )
    ...
    def read_gradient(self, gradient_files):
        software = self.softwares["software"]
        en, grad, dipole, atids, coords = software.read.gradient(
            self.config, **gradient_files
        )
    ...
    def _write_gradient(
        self, software, file, job_name, coords, atnums, extra_info=False
    ):
    ...
    def write_scan(
        ...
        if self.config.dihedral_scanner == "relaxed_scan":
            ...
            software.write.scan_torsiondrive(
                file,
                scan_id,
                self.config,
                coords,
    ...
    def _get_qm_softwares(self, config):
        ...
        if (
            scanner == "torsiondrive"
            and softwares["scan_software"].has_torsiondrive is False
        ):
            self.logger.error(
                "TorsionDrive not supported for scan_software "
    ...
    def _set_qm_software(self, selection):
        try:
            software = implemented_qm_software[selection.value](
    ...
    def _register_method(self):
        ...
        method = {
            key: val for key, val in self.config.__dict__.items() if key in method_list
        }
        method.update(
            {key: val.upper() for key, val in method.items() if isinstance(val, str)}
        )
