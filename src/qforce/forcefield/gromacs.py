...
class Gromacs(ForcefieldSettings):
    ...
    def write_top(self, directory):
        with open(f"{directory}/gas.top", "w") as top:
            ...
            top.write(
                f"{1:>8} {self.ff.comb_rule:>12} {'yes':>12} {self.ff.fudge_lj:>12} "
    ...
    def write_gro(self, directory, coords, box=[20.0, 20.0, 20.0]):
        ...
        with open(f"{directory}/gas.gro", "w") as gro:
            ...
            for i, (a_name, coord) in enumerate(
                zip(self.ff.atom_names, coords_nm), start=1
            ):
    ...
    def write_itp_atoms_and_molecule(self, itp):
        ...
        if self.ff.comb_rule == 1:
            itp.write(
                ";   name  at_num     mass   charge  type           c6          c12\n"
            )
        else:
            itp.write(
        ...
        for i, (lj_type, a_name, q, mass) in enumerate(
            ...
            itp.write(
                f"{i:>5} {lj_type:>9} {1:>6} {self.ff.residue:>6} {a_name:>7} {i:>5} "
            )
    ...
    def write_itp_angles(self, itp):
        ...
        for angle in self.ff.terms["angle"]:
            ...
            if not self.ff.urey or len(urey) == 0:
                ...
                itp.write(
                    f"{ids[0]:>6} {ids[1]:>6} {ids[2]:>6} {5:>6} {equ:>12.5f} "
        ...
        if "cross_bond_bond" in self.ff.terms:
            ...
            for cross_bb in self.ff.terms["cross_bond_bond"]:
                ...
                itp.write(
                    f"{ids[0]:>6} {ids[1]:>6} {ids[2]:>6} {3:>6} {equ1:>12.7f} {equ2:>12.7f} "
                    f"{fconst:>10.1f} \n"
                )
        ...
        if "cross_bond_angle" in self.ff.terms:
            ...
            for term in self.ff.terms["cross_bond_angle"]:
                print(
                    term.atomids + 1, np.degrees(term.equ[0]), term.equ[1], -term.fconst
                )
        ...
        if "cross_angle_angle" in self.ff.terms:
            ...
            for term in self.ff.terms["cross_angle_angle"]:
                print(
                    term.atomids + 1,
                    np.degrees(term.equ[0]),
                    np.degrees(term.equ[1]),
                    -term.fconst,
                )
    ...
    def write_itp_dihedrals(self, itp):
        ...
        # rigid dihedrals
        if (
            "dihedral/rigid" in self.ff.terms
            and len(self.ff.terms["dihedral/rigid"]) > 0
        ):
        ...
        # improper dihedrals
        if (
            "dihedral/improper" in self.ff.terms
            and len(self.ff.terms["dihedral/improper"]) > 0
        ):
        ...
        for dihed in self.ff.terms["dihedral/flexible"]:
            ...
            itp.write(
                f"{ids[0]:>6} {ids[1]:>6} {ids[2]:>6} {ids[3]:>6} {3:>6} {c[0]:>11.3f} "
            )
            itp.write(
                f"{c[1]:>11.3f} {c[2]:>11.3f} {c[3]:>11.3f} {c[4]:>11.3f} {c[5]:>11.3f}\n"
            )

        # inversion dihedrals
        if (
            "dihedral/inversion" in self.ff.terms
            and len(self.ff.terms["dihedral/inversion"]) > 0
        ):
    ...
    def add_restraints(self, restraints, directory, fc=1000):
        with open(f"{directory}/{self.ff.mol_name}_qforce.itp", "a") as itp:
            ...
            for restraint in restraints:
                ...
                itp.write(
                    f"{a1:>5} {a2:>5} {a3:>5} {a4:>5} {1:>5} {phi:>10.4f}  0.0  {fc}\n"
                )
