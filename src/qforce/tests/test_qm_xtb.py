...
class TestReadHessian:
    ...
    def test_coords(self, hessian):
        ...
        assert all(
            np.isclose(
                coords[0, :],
                [-5.47755155927663, 1.91096783090613, -0.07170582428722],
                rtol=0.01,
            )
        )
...
class TestReadScan:
    ...
    def scan():
        ...
        (n_atoms, coords, angles, energies, point_charges) = ReadxTB(config).scan(
            config, xTB_default["fragments_coord"]
        )
    ...
    def test_coords(self, scan):
        ...
        assert all(
            np.isclose(
                coords[0][0],
                [-5.47742873919790, 1.91100460922811, -0.07168838231625],
                rtol=0.01,
            )
        )
