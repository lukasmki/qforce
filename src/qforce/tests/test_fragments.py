...
frag_lib = {}/qforce_fragments
    ...
    run(
        input_arg=tmpdir.join("propane.xyz").strpath,
        config=tmpdir.join("settings").strpath,
    )
    # add hessian output
    tmpdir.join("propane_qforce").join("1_hessian").join(
        "propane_3fbff44995be158b3fd2daeef2df6f33_hessian.log"
    ).mksymlinkto(Gaussian_default["out_file"])
    tmpdir.join("propane_qforce").join("1_hessian").join(
        "propane_3fbff44995be158b3fd2daeef2df6f33_hessian.fchk"
    ).mksymlinkto(Gaussian_default["fchk_file"])
    ...
    assert (
        tmpdir.join("propane_qforce")
        .join("2_fragments")
        .join("CC_H8C3_d91b46644317dee9c2b868166c66a18c~1")
        .join("CC_H8C3_d91b46644317dee9c2b868166c66a18c~1.inp")
        .isfile()
    )
