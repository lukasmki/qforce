...
class MChem(ForcefieldSettings):
    ...
    def write_residues(self, forcefield):
        ...
        for bond in self.ff.terms["bond"]:
            ET.SubElement(
                residue,
                "Bond",
                {"from": str(bond.atomids[0] + 1), "to": str(bond.atomids[1] + 1)},
            )
    ...
    def write_multipole_term(self, term, writer):
        ...
        ET.SubElement(
            ...
            "Multipole",
            {
                ...
                "class4": str(ids[3]),
                "center": str(center),
                "frametype": term.frame_type,
                "q00": str(term.q),
                "q10": str(term.dipole_spher[0]),
                "q11c": str(term.dipole_spher[1]),
                "q11s": str(term.dipole_spher[2]),
                "q20": str(term.quad_spher[0]),
                "q21c": str(term.quad_spher[1]),
                "q21s": str(term.quad_spher[2]),
                "q22c": str(term.quad_spher[3]),
                "q22s": str(term.quad_spher[4]),
            },
        )
    ...
    def write_harmonic_bond_term(self, term, writer):
        ...
        k = str(round(term.fconst, 5))
        ET.SubElement(
            writer, "Bond", {"class1": ids[0], "class2": ids[1], "length": equ, "k": k}
        )
    ...
    def write_morse_bond_term(self, term, writer):
        ...
        k = str(round(term.fconst, 5))
        e_dis = str(
            self.ff.bond_dissociation_energies[term.atomids[0], term.atomids[1]]
        )
        ET.SubElement(
            writer,
            "Bond",
            {"class1": ids[0], "class2": ids[1], "length": equ, "k": k, "e_dis": e_dis},
        )
