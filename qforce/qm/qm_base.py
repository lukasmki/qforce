...
class QMInterface(Colt):
    ...
    def _dct_to_hash(settings):
        return hashlib.md5(
            "".join(
                f"{key.lower()}:{str(value).lower()}" for key, value in settings.items()
            ).encode()
        ).hexdigest()
    ...
    def run(self, calculation, ncores):
        ...
        if self.name != calculation.software:
            raise CalculationFailed(
                f"Do not know software '{calculation.software}', "
        ...
        with calculation.within():
            ...
            for command in commands:
                try:
                    ...
                    raise CalculationFailed(
                        f"subprocess registered error: '{err}' "
                        f"in '{os.getcwd()}'"
        ...
        try:
            ...
            raise CalculationFailed(
                "Not all necessary files could be generated for calculation"
...
class WriteABC(ABC):
    ...
    def _scan_torsiondrive_helper(self, file, job_name, config, scanned_atoms, engine):
        ...
        file.write(
            f"torsiondrive-launch {job_name}_input.xyz "
...
class HessianOutput:
    def __init__(
        ...
        self.energy = energy
        self.hessian = (
            self.check_type_and_shape(
                hessian, "hessian", float, (((n_atoms * 3) ** 2 + n_atoms * 3) / 2,)
            )
            * vib_scaling**2
        )
        self.b_orders = self.check_type_and_shape(
            b_orders, "b_orders", float, (n_atoms, n_atoms)
        )
        self.n_bonds = self.b_orders.sum(axis=1).round().astype(int)
        self.point_charges = self.check_type_and_shape(
            point_charges, "point_charges", float, (n_atoms,)
        )
        ...
        if lone_e is not None or n_bonds is not None:
            warn(
                'HessianOutput no longer needs "lone_e" or "n_bonds" arguments and they will be '
    ...
    def check_type(value, name, expected_type):
        if not isinstance(value, expected_type):
            sys.exit(
                f'WARNING: A valid "{name}" property was not found in the hessian output'
                " file(s). Exiting...\n\n"
            )
        return value
    ...
    def check_type_and_shape(value, name, expected_type, expected_shape):
        ...
        if value.size == 0:
            sys.exit(
                f'ERROR: No data found in the QM Hessian output file(s) for "{name}".'
                " Exiting...\n\n"
            )
        elif value.dtype != np.dtype(expected_type):
            raise TypeError(
                f'"{name}" property expected a type of "{np.dtype(expected_type)}",'
                f' but got "{value.dtype}" for the QM Hessian output.'
            )
        elif value.shape != expected_shape:
            sys.exit(
                f'ERROR: "{name}" property expected a shape of "{expected_shape}", but got '
...
class ScanOutput:
    ...
    def __init__(
        self, file, n_steps, n_atoms, coords, angles, energies, dipoles, charges
    ):
        self.n_atoms = n_atoms
        self.n_steps = n_steps
        self.forces = np.zeros((n_steps, n_atoms, 3))
        angles, energies, coords, dipoles, self.charges, self.mismatch = (
            self.check_shape(angles, energies, coords, dipoles, charges, file)
        )
        self._angles, self._energies, self.coords, self.dipoles = self._rearrange(
    ...
    def check_shape(self, angles, energies, coords, dipoles, charges, file):
        ...
        angles, coords, energies, dipoles = (
            np.asarray(angles),
            np.asarray(coords),
            np.asarray(energies),
            np.asarray(dipoles),
        ...
        for prop, name, shape in [
            (angles, "angles", (self.n_steps,)),
        ...
        if mismatched:
            print(
                f"WARNING: {mismatched} properties have missing/extra data in the file:\n{file}"
            )
