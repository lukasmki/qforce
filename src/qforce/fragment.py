...
def fragment(mol, qm, job, config):
    ...
    check_and_notify(
        job, config.scan, len(unique_dihedrals), len(fragments), len(generated)
    )
    ...
    if n_unique == 0:
        ...
        if n_missing == 0:
            ...
            if n_missing - n_generated > 0:
                job.logger.info(
                    f"QM input files for them are created in: {job.frag_dir}"
                )
...
class Fragment:
    ...
    def check_fragment(self, job, config, mol, qm, scanner):
        ...
        if scanner == "torsiondrive":
            self.calc = job.Calculation(
                f"{self.id}_torsiondrive.inp",
    ...
    def identify_fragment(self, mol, config):
        ...
        next_neigh = [
            [a, n]
            for a in self.atomids
            for n in mol.topo.neighbors[0][a]
            if n not in self.atomids
        ]
        while next_neigh != []:
            new = []
            for a, n in next_neigh:
                ...
                if n in self.atomids:
                    ...
                    config.frag_threshold < 1  # fragmentation turned off
                    or n_neigh < config.frag_threshold  # don't break first n neighbors
                    or bond["order"]
                    >= config.conj_bo_cutoff  # don't break bonds conjugated more than 1.4 (default)
                    or ELE_ENEG[mol.atomids[a]]
                    > 3  # don't break if very electronegative
                    or (config.break_co_bond and ELE_ENEG[mol.atomids[n]] > 3)
                    or mol.topo.n_neighbors[n] == 1  # don't break terminal atoms
                    or bond["n_rings"]
                    > 1  # don't break a bond that is in multiple rings
                    or (
                        bond["n_rings"] == 1
                        and self.check_single_ring_rules(mol, bond, a, n)
                    )
                ):
                    new.append(n)
    ...
    def make_fragment_graph(self, mol):
        self.map_mol_to_frag = {
            self.atomids[i]: i for i in range(self.n_atoms_without_cap)
        }
        self.scanned_atomids = [self.map_mol_to_frag[a] for a in self.scanned_atomids]
        self.elements = [
            mol.atomids[idx] for idx in self.atomids + [cap["idx"] for cap in self.caps]
        ]
        ...
        for cap in self.caps:
            ...
            self.graph.add_node(
                self.n_atoms_without_cap + cap["n_cap"],
                elem=1,
                n_bonds=1,
                coords=cap["coord"],
                capping=True,
            ...
            self.graph.add_edge(
                self.n_atoms_without_cap + cap["n_cap"],
                self.map_mol_to_frag[cap["connected"]],
                type=h_type,
            )
    ...
    def make_fragment_identifier(self, config, mol, qm):
        ...
        if (
            config.frag_threshold < 1
        ):  # If fragmentation is off - take molecule's charge&multi
        ...
        self.hash = frag_id = (
            frag_id.replace("(", "-").replace(",", "").replace(")", "")
        )
    ...
    def check_for_fragment(self, job, config, qm):
        ...
        nm = iso.categorical_node_match(
            ["elem", "n_bonds", "capping", "scan"], [0, 0, 0, False, False]
        )
        em = iso.categorical_edge_match(["type"], [0])

        os.makedirs(self.dir, exist_ok=True)
        identifiers = [
            i for i in sorted(os.listdir(f"{self.dir}")) if i.startswith("ident")
        ]
        ...
        for id_no, id_file in enumerate(identifiers, start=1):
            ...
            if (
                self.graph.graph["qm_method"] == compared.graph["qm_method"]
                and GM.is_isomorphic()
            ):
    ...
    def check_new_scan_data(self, job, mol, config, qm):
        files = [
            ...
            if f.startswith(self.id) and f.endswith(("log", "out"))
        ]

        if files:
            self.has_data = True
            qm_out = qm.read_scan(self.folder, files)
            qm_out = qm.do_scan_sp_calculations(
                self.folder, self.id, qm_out, mol.atomids
            )
            # get qm out energies
    ...
    def make_fragment_terms(self, mol):
        ...
        self.terms = mol.terms.subset(
            self.atomids, map_mol_to_db, remove_non_bonded=self.remove_non_bonded
        )
        self.non_bonded = mol.non_bonded.subset(
            mol.non_bonded, self.frag_charges, map_mol_to_db
        )
        ...
        for n in range(3):
            for i in range(self.n_atoms):
                ...
                self.neighbors[n].append(
                    [
                        map_mol_to_db[neigh]
                        for neigh in neighs
                        if neigh in map_mol_to_db.keys()
                        and map_mol_to_db[neigh] < self.n_atoms
                    ]
                )

    def check_for_qm_data(self, job, config, mol, qm):
        if self.has_data:
            self.qm_energies = np.loadtxt(
                f"{self.dir}/scandata_{self.hash_idx}", unpack=True
            )[1]
            ...
            with open(f"{self.dir}/qm_method_{self.hash_idx}", "w") as file:
                if not isinstance(self.graph.graph["qm_method"]["software"], str):
                    self.graph.graph["qm_method"]["software"] = self.graph.graph[
                        "qm_method"
                    ]["software"].value
    ...
    def assign_frag_charge(self, mol, charges):
        if self.charge_method in charges.keys() and not (
            self.ext_charges and self.use_ext_charges_for_frags
        ):
