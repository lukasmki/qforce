        ...
        with open(out_file, "r", encoding="utf-8") as file:
            for line in file:
                if "Charge Model 5" in line and self.config.charge_method == "cm5":
                    ...
                    "Merz-Kollman RESP Net Atomic" in line
                    and self.config.charge_method == "resp"
                ):
...
class WriteQChem(WriteABC):
    ...
    def opt(self, file, job_name, settings, coords, atnums):
        self._write_molecule(
            file, job_name, atnums, coords, settings.charge, settings.multiplicity
        )
        self._write_job_setting(
            file, job_name, settings, self.hess_opt_rem, is_opt=True
        )
        file.write("\n\n\n\n\n")
    ...
    def hessian(self, file, job_name, settings, coords, atnums):
        ...
        file.write("$molecule\n  read\n$end\n\n")
        self._write_job_setting(
            file, job_name, settings, self.hess_freq_rem, is_opt=True
        )
        file.write("\n$nbo\n  nbo\n  bndidx\n$end\n\n")
    ...
    def _write_scan_info(file, scanned_atoms, start_angle, final_angle, scan_step_size):
        ...
        file.write("\n$scan\n")
        file.write(
            f"  tors {a1} {a2} {a3} {a4} {start_angle:.3f} {final_angle:.3f} "
            f"{scan_step_size:.2f}\n"
        )
        file.write("$end\n\n")
    ...
    @staticmethod
    def _write_molecule(
        file, job_name, atnums, coords, charge, multiplicity, extra_info=False
    ):
        file.write("$comment\n")
