...
class NonBonded:
    ...
    def from_topology(cls, config, job, qm_out, topo, ext_q, ext_lj):
        ...
        if config.n_excl == 2:
            pairs, exclusions = cls._set_1_4_pairs_and_exclusions(
                topo, exclusions, pairs
            )
        ...
        if config.do_multipole:
            if not qm_out.fchk_file:
                raise KeyError(
                    "QM method does not have fchk file - This is not supported for GDMA"
                )
            # dipoles and quads are averaged later
            q, dipole, quadrupole = compute_gdma(
                job, config.gdma_exec, qm_out.fchk_file
            )
        elif ext_q:
            q = np.array(ext_q)
            ...
            if config.charge_scaling != 1.0 and qm_out.charge != 0:
                job.logger.warning(
                    "The system has a net charge and therefore point charges "
                ...
                job.logger.note(
                    f"QM atomic charges are scaled by {config.charge_scaling} to "
        ...
        lj_pairs, lj_1_4, lj_atomic_number = set_external_lennard_jones(
            job, config, comb_rule, lj_types, ext_lj, h_cap
        )
    ...
    def subset(cls, non_bonded, frag_charges, mapping):
        ...
        quadrupole = np.array(
            [non_bonded.quadrupole[rev_map[i]] for i in range(n_atoms)]
        )

        lj_types = [non_bonded.lj_types[rev_map[i]] for i in range(n_atoms)]
        lj_pairs = {
            key: val
            for key, val in list(non_bonded.lj_pairs.items())
            if key[0] in lj_types + [h_cap] and key[1] in lj_types + [h_cap]
        }
        lj_1_4 = {
            key: val
            for key, val in list(non_bonded.lj_1_4.items())
            if key[0] in lj_types + [h_cap] and key[1] in lj_types + [h_cap]
        }
        lj_atomic_number = {
            key: val
            for key, val in list(non_bonded.lj_atomic_number.items())
            if key in lj_types + [h_cap]
        }
        exclusions = [
            (mapping[excl[0]], mapping[excl[1]])
            for excl in non_bonded.exclusions
            if excl[0] in mapping.keys() and excl[1] in mapping.keys()
        ]

        pairs = [
            (mapping[pair[0]], mapping[pair[1]])
            for pair in non_bonded.pairs
            if pair[0] in mapping.keys() and pair[1] in mapping.keys()
        ]
        ...
        if value:
            for line in value.split("\n"):
                ...
                if len(line) > 1:
                    ...
                    logger.warning(
                        "Exclusion/Pair lines should contain at least two atom IDs:\n"
    ...
    def _set_1_4_pairs_and_exclusions(topo, exclusions, pairs):
        for i in range(topo.n_atoms):
            for neigh in topo.neighbors[2][i]:
                if (
                    i < neigh
                    and [i, neigh] not in pairs
                    and (i, neigh) not in exclusions
                ):
...
def get_external_lennard_jones(config, topo, q, job, ext_lj):
    if config.lennard_jones == "gromos_auto":
        ...
        if "lj_types" not in ext_lj:
            sys.exit(
                'ERROR: You have not provided the "lj_types" list in the "ext_lj" '
                "dictionary.\n"
            )
        elif len(ext_lj["lj_types"]) != topo.n_atoms:
            sys.exit(
                "ERROR: Provided number of Lennard-Jones parameters does not match number "
                "of atoms.\n"
            )
        else:
        ...
        if os.path.isfile(lj_file):
            lj_types = np.loadtxt(
                f"{job.dir}/ext_lj", dtype="str", comments=["#", ";"]
            ).ravel()
        else:
            job.logger.error(
                f"Manual LJ types requested ({config.lennard_jones}) but the "
        ...
        if lj_types.size != topo.n_atoms:
            job.logger.error(
                'Format of your "ext_lj" file is wrong.\nPlease provide there '
...
def set_non_bonded_props(config):
    if config.lennard_jones == "ext":
        if (
            not config.ext_lj_fudge
            or not config.ext_q_fudge
            or not config.ext_comb_rule
            or not config.ext_h_cap
        ):
            sys.exit(
                "ERROR: External set of Lennard-Jones interactions requested but not all "
...
class Neighbors(list):
    ...
    def count(
        ...
        for neigh in neighs:
            if (
                (elem is None or neigh.elem == elem)
                and (b_order_gt is None or neigh.b_order > b_order_gt)
                and (b_order_lt is None or neigh.b_order < b_order_lt)
                and (n_bonds_gt is None or neigh.n_bonds > n_bonds_gt)
                and (in_ring is None or in_ring)
            ):
...
def determine_opls_atom_types(logger, topo, q):
    ...
    logger.note(
        "Automatic atom-type determination (used only for LJ interactions) is new. \n"
    ...
    for atomid, elem in enumerate(topo.atomids):
        ...
        if elem == 1:
            if neighs.count(elem=6):
                ...
                if second_neighs.count(
                    elem=6, b_order_gt=1.25, b_order_lt=1.6, in_ring=True
                ):
                    a_type = "opls_146"  # HA
            ...
            if neighs.count(elem=8, b_order_gt=1.25) > 1:
                ...
                neighs.count(b_order_gt=1.25) or neighs.count(elem=16, n_bonds_gt=2) > 0
            ):
                a_type = "opls_237"  # N
            else:
                a_type = "opls_900"  # NT
            ...
            sys.exit(
                f"ERROR: Atomic number {elem} (encountered for atom {atomid + 1}) is "
...
def determine_gromos_atom_types(logger, topo, q):
    ...
    for i, elem in enumerate(topo.atomids):
        ...
        if elem == 1:
            ...
            in_ring_and_conj = [
                all([topo.edge(*edge)["order"] >= 1.25, topo.edge(*edge)["in_ring"]])
                for edge in topo.graph.edges(i)
            ]
            united_charge = q[i] + sum(
                [q[j] for j in topo.neighbors[0][i] if topo.atomids[j] == 1]
            )
...
def set_qforce_lennard_jones(topo, comb_rule, lj_a, lj_b):
    ...
    for comb in combinations_with_replacement(lj_type_dict.keys(), 2):
        ...
        params = use_combination_rule(
            lj_type_dict[comb[0]], lj_type_dict[comb[1]], comb_rule
        )
...
def set_external_lennard_jones(job, config, comb_rule, lj_types, ext_lj, h_cap):
    ...
    if config.lennard_jones == "ext" and ext_lj:
        (atom_types, nonbond_params, nonbond_1_4, atomic_numbers) = (
            set_external_lennard_jones_from_dict(ext_lj)
        )
    else:
        (atom_types, nonbond_params, nonbond_1_4, atomic_numbers) = (
            read_ext_nonbonded_file(config, job.md_data)
        )

    for lj_type in lj_types:
        if lj_type not in atom_types.keys():
            sys.exit(
                f"ERROR: The atom type you have entered ({lj_type}) is not in the FF library "
    ...
    for comb in combinations_with_replacement(set(list(lj_types) + [h_cap]), 2):
        ...
        if comb in nonbond_params.keys():
            ...
            params = use_combination_rule(
                atom_types[comb[0]], atom_types[comb[1]], comb_rule
            )
...
def read_ext_nonbonded_file(config, md_data):
    ...
    if config.lennard_jones == "ext":
        if config.ext_lj_lib:
            ...
            sys.exit(
                "ERROR: External set of Lennard-Jones interactions requested but the \n"
...
def sum_charges_to_qtotal(logger, topo, q):
    ...
    if extra != 0:
        ...
        if prob.status == 1:
            ...
            logger.info(
                "Failed to equate total of charges to the total charge of "
