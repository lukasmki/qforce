    ...
    def get_subset(self, fragment, mapping, ignore=[]):
        ...
        for name, term in self.ho_items():
            ...
            if isinstance(term, MultipleTermStorge):
                key_ignore = [
                    self.get_key_subkey(ignore_key)[1]
...
class TermStorage(UserList):
    ...
    def fulfill(self, name, atomids=None, notthis=False):
        if atomids is None:
            ...
            def condition(term):
                return term == name and all(
                    termid == idx for termid, idx in zip(term.atomids, atomids)
                )
    ...
    def get_subset(self, fragment, mapping, remove_non_bonded=[]):
        ...
        for term in self:
            if set(term.atomids).issubset(fragment) and (
                term.name != "NonBondedTerm"
                or all([idx not in remove_non_bonded for idx in term.atomids])
            ):
