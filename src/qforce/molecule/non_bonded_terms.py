...
class NonBondedTerms(TermBase):
    ...
    def _get_terms(cls, topo, non_bonded):
        ...
        inv_eps0 = (
            1 / (4 * np.pi * _eps0) * m / J / kJ * mol
        )  # from F m-1 to kJ mol−1 Angstrom e−2

        for i in range(topo.n_atoms):
            for j in range(i + 1, topo.n_atoms):
                close_neighbor = any(
                    [j in topo.neighbors[c][i] for c in range(non_bonded.n_excl)]
                )

                if not close_neighbor and (i, j) not in non_bonded.exclusions:
                    pair_name = tuple(
                        sorted([non_bonded.lj_types[i], non_bonded.lj_types[j]])
                    )
                    ...
                    if (i, j) in non_bonded.pairs:
                        if pair_name in non_bonded.lj_1_4.keys():
                            ...
                            param = [
                                p * non_bonded.fudge_lj
                                for p in non_bonded.lj_pairs[pair_name]
                            ]
                        param.append(
                            non_bonded.q[i]
                            * non_bonded.q[j]
