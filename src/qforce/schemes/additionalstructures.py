...
class StructuresFromFile(CustomStructureCreator):
    ...
    def __init__(self, weight, folder, en_struct, grad_struct, hess_struct):
        ...
        self._energy = EnergyCalculationIterCreator(
            folder / "en_structs", weight, coords_ids_iter(en_struct)
        )
        self._grad = GradientCalculationIterCreator(
            folder / "grad_structs", weight, coords_ids_iter(grad_struct)
        )
        self._hess = HessianCalculationIterCreator(
            folder / "hess_struct", weight, coords_ids_iter(hess_struct)
        )

    @classmethod
    def from_config(cls, config, folder):
        if (
            not config["en_struct"]
            and not config["grad_struct"]
            and not config["hess_struct"]
        ):
            return None
        return cls(
            config["weight"],
