...
class UreyBradleyTerm(TermBase):
    ...
    def _get_terms(cls, topo, non_bonded):
        ...
        for a1, a2, a3 in topo.angles:
            ...
            if theta < 2.9671 and (
                not topo.edge(a2, a1)["in_ring3"] or not topo.edge(a2, a3)["in_ring3"]
            ):
...
def get_bond_dissociation_energies(md_data, mol):
    ...
    for a1, a2, props in mol.topo.graph.edges(data=True):
        ...
        if props["type"] not in bde_dict:
            raise ValueError(
                "Morse potential chosen, but dissociation energy not known for this atom number pair "
        ...
        if mol.atomids[a1] == mol.atomids[a2]:
            e_dis2, n_matches2 = choose_bond_type(
                bde_dict[props["type"]], neighs1, neighs2, 1, 0
            )
...
def read_bond_dissociation_neighbors(a, neighs):
    ...
    if neighs != "*" and neighs != "":
        ...
        for neigh in neighs:
            ...
            neighs_formatted.append(
                f"{min(a, a_neigh)}({float(bo_neigh):.1f}){max(a, a_neigh)}"
            )
