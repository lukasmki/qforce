...
class TestReadHessian:
    ...
    def hessian():
        ...
        (
            ...
            b_orders,
            point_charges,
        ) = ReadGaussian(config).hessian(
            config, Gaussian_default["out_file"], Gaussian_default["fchk_file"]
        )
    ...
    def test_coords(self, hessian):
        ...
        assert all(
            np.isclose(coords[0, :], [-1.277008, -0.260352, 0.000062], rtol=0.01)
        )
...
class TestReadScan:
    ...
    def scan():
        config = GaussianConfig.from_questions(check_only=True)
        (n_atoms, coords, angles, energies, point_charges) = ReadGaussian(config).scan(
            config, Gaussian_default["fragments"]
        )
    ...
    def test_energies(self, scan):
        (n_atoms, coords, angles, energies, point_charges) = scan
        energy = (
            "-118.965912,-118.9652606,-118.9636263,-118.9618925,-118.96"
