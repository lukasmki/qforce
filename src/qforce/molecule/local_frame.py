...
class LocalFrameTermABC(TermABC):
    ...
    def __init__(
        self, frame_type, atomids, q, dipole, quadrupole, coords, center, a_type
    ):
        ...
        self.equ = None
        self.fconst = None
        self.dipole, self.quadrupole = self.convert_multipoles_to_local_frame(
            coords, dipole, quadrupole
        )
        self.dipole_spher, self.quad_spher = (
            self.convert_multipoles_to_local_spherical_frame()
        )
    ...
    def convert_multipoles_to_local_cartesian_frame(self):
        dipoles = np.array(
            [self.dipole_spher[1], self.dipole_spher[2], self.dipole_spher[0]]
        )
...
class ZOnlyTerm(LocalFrameTermABC):
    def compute_rotation_matrix(self, coords):
        ...
        if len(coords) == 3:
            ...
            dir2 = np.array(
                [vec12[1], vec12[2], vec12[0]]
            )  # lazy but probably good enough?
...
class LocalFrameTerms(TermFactory):
    ...
    def _get_terms(cls, topo, non_bonded, termtypes):
        ...
        if (
            np.abs(non_bonded.quadrupole).sum() == 0
            and np.abs(non_bonded.dipole).sum() == 0
        ):
            return terms
        ...
        for i, node in topo.graph.nodes(data=True):
            if node["n_neighs"] == 1:  # terminal atoms
                ...
                other_neighs = [
                    neigh
                    for neigh in topo.node(node["neighs"][0])["neighs"]
                    if neigh != i
                ]  # 1st neigh's
                # Diatomic molecule
                if topo.n_atoms == 2:
                ...
                # Terminal atom, 1st neighbor has 3-neighbors
                    ...
                    if (
                        neigh["hybrid"] == "planar"
                        and other_neighs[0]["type"] == other_neighs[1]["type"]
                    ):
                ...
                # Terminal atom, 1st neighbor has 4-neighbors
                    ...
                    # Tetrahedral with 3 atoms the same, like F on CFH3
                        ...
                        neigh["n_unique_neighs"] == 2
                        and neigh["n_nonrepeat_neighs"] == 1
                    ...
                    # Tetrahedral with 2x2 atoms the same, like F on CF2H2
                        ...
                        neigh["n_unique_neighs"] == 2
                        and neigh["n_nonrepeat_neighs"] == 0
                    ):
                        chosen = [
                            group for group in neigh["unique_neighs"] if i in group
                        ]
                        ...
                        neigh["n_unique_neighs"] == 3
                        and neigh["n_nonrepeat_neighs"] == 2
                    ):
                        group_with_two = [
                            group for group in neigh["unique_neighs"] if len(group) == 2
                        ]
                        # Tetrahedral with 2 atoms the same, like H on CFClH2
                        if i in group_with_two:
                            other_same_atom = [
                                atom for atom in group_with_two if i != atom
                            ][0]
                            atomids = [i, node["neighs"][0], other_same_atom]
                            add_term(
                                "z_then_x",
                        ...
                        # Tetrahedral with 2 atoms the same, like F on CFClH2
                            ...
                            other_diff_atom = [
                                atom
                                for atom in other_neighs
                                if atom not in group_with_two
                            ][0]
                ...
                # Pyramidal with 2 unique neighbors, like N on methylamine
                    ...
                    lone = [
                        unique for unique in node["unique_neighs"] if len(unique) == 1
                    ][0]
                    equal_pair = [
                        unique for unique in node["unique_neighs"] if len(unique) == 2
                    ][0]
                ...
                # Tetrahedral center, 3 same atoms, like C in CH3F or C in ethane
                    ...
                    other_atoms = [
                        atom for atom in node["neighs"] if atom != unique_atom
                    ]
                ...
                # Tetrahedral center, 2x2 same atoms, like C in CH2F2 or central carbon in propane
                    ...
                    connected_pair = [
                        group
                        for group in node["unique_neighs"]
                        if most_connected_neigh in group
                    ]
                ...
                # Tetrahedral center, 2 same atoms, like C in CH2FCl
                    ...
                    atomids = [
                        i,
                        node["nonrepeat_neighs"][0],
                        node["nonrepeat_neighs"][1],
                    ]
                    add_term(
                        "z_then_x",
                        non_bonded,
    ...
    def average_equivalent_terms(terms):
        ...
        for term in terms:
            ...
            term.dipole, term.quadrupole = (
                term.convert_multipoles_to_local_cartesian_frame()
            )
