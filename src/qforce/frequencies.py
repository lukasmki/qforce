...
def calc_qm_vs_md_frequencies(job, qm, md_hessian):
    ...
    mean_percent_error = write_vibrational_frequencies(
        qm_freq, qm_vec, md_freq, md_vec, qm, job
    )
...
def calc_vibrational_frequencies(upper, qm):
    ...
    # mass weight
    masses = [ATOMMASS[qm.atomids[i]] for i in range(qm.n_atoms)]
    ...
    with open(freq_file, "w") as f:
        ...
        for i, (q, m) in enumerate(zip(qm_freq, md_freq)):
            ...
            if q > 100:
                errors.append(err)
            f.write(
                f"{i + transrot + 1:>4}{q:>12.3f}{m:>12.3f}{diff:>12.3f}{err:>8.2f}\n"
            )
