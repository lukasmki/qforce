...
class OpenMM(ForcefieldSettings):
    ...
    def write(self, directory, coords, box=[20.0, 20.0, 20.0]):
    ...
    def write_pdb(self, directory, coords, box):
        with open(f"{directory}/{self.ff.mol_name}.pdb", "w") as file:
            file.write(
                f"CRYST1{box[0]:9.3f}{box[1]:9.3f}{box[2]:9.3f}{90.0:7.2f}{90.0:7.2f}{90.0:7.2f} P 1\n"
            )
            file.write("MODEL     1\n")

            for i, (coord, name, symbol) in enumerate(
                zip(coords, self.ff.atom_names, self.ff.symbols), start=1
            ):
                file.write(
                    f"ATOM  {i:5d} {name:4s} {self.ff.residue:4s} {1:4d}    "
    ...
    def write_xml(self, directory, box):
        system = ET.Element(
            "System", {"openmmVersion": "8.1", "type": "System", "version": "1"}
        )
    ...
    def write_harmonic_bond_term(self, term, writer):
        ...
        ET.SubElement(
            writer, "Bond", {"p1": str(ids[0]), "p2": str(ids[1]), "d": equ, "k": k}
        )
    ...
    def write_harmonic_angle_term(self, term, writer):
        ...
        ET.SubElement(
            writer,
            "Angle",
            {"p1": str(ids[0]), "p2": str(ids[1]), "p3": str(ids[2]), "a": equ, "k": k},
        )
    ...
    def write_cross_bond_cos_angle_header(self, forces):
        ba_cross_eq = (
            "max(k*(cos(angle(p1,p2,p3))-cos(theta0))*(distance(p4,p5)-r0), -20)"
        )
        ba_force = ET.SubElement(
            forces,
            "Force",
    ...
    def write_cross_angle_angle_header(self, forces):
        aa_cross_eq = (
            "max(k*(angle(p1,p2,p3)-theta1_0)*(angle(p4,p5,p6)-theta2_0), -20)"
        )
        aa_force = ET.SubElement(
            forces,
            "Force",
            {
                "energy": aa_cross_eq,
    ...
    def write_cross_dihed_angle_header(self, forces):
        da_cross_eq = (
            "k * (1+cos(n*dihedral(p1,p2,p3,p4)-phi0)) * (angle(p5,p6,p7)-theta0)"
        )
        da_force = ET.SubElement(
            forces,
            "Force",
            {
                "energy": da_cross_eq,
    ...
    def write_harmonic_dihedral_header(self, forces):
        ...
        imp_dih_force = ET.SubElement(
            forces,
            "Force",
            {
                "energy": harm_dih_eq,
                "name": " HarmonicDihedral",
                "usesPeriodic": "0",
                "type": "CustomTorsionForce",
                "forceGroup": "0",
                "version": "3",
            },
        )
    ...
    def write_inversion_dihedral_header(self, forces):
        ...
        inv_dih_force = ET.SubElement(
            forces,
            "Force",
            {
                "energy": inv_dih_eq,
                "name": "InversionDihedral",
                "usesPeriodic": "0",
                "type": "CustomTorsionForce",
                "forceGroup": "0",
                "version": "3",
    ...
    def write_multipole_header(self, forces):
        force = ET.SubElement(
            ...
            "Force",
            {
                "type": "AmoebaMultipoleForce",
                "name": "AmoebaMultipoleForce",
                "forceGroup": "0",
                "version": "4",
                "aEwald": "0",
                "cutoffDistance": "1",
                "ewaldErrorTolerance": ".0001",
        ...
        ET.SubElement(
            force, "MultipoleParticleGridDimension", {"d0": "0", "d1": "0", "d2": "0"}
        )
        ET.SubElement(
            force,
            "ExtrapolationCoefficients",
            {"c0": "-.154", "c1": ".017", "c2": ".658", "c3": ".474"},
        )
    ...
    def write_multipole_term(self, term, writer):
        ...
        part = ET.SubElement(
            ...
            "Particle",
            {
                "axisType": axis_types[term.frame_type],
                "multipoleAtomZ": str(ids[0]),
                "multipoleAtomX": str(ids[1]),
                "multipoleAtomY": str(ids[2]),
                "charge": str(term.q),
                "damp": "0",
                "polarity": "0",
                "thole": "0",
        ...
        ET.SubElement(
            part, "Dipole", {"d0": str(dips[0]), "d1": str(dips[1]), "d2": str(dips[2])}
        )
    ...
    def write_coulomb(self, forces):
        nb_force = ET.SubElement(
            ...
            "Force",
            {
                ...
                "forceGroup": "0",
                "alpha": "0",
                "dispersionCorrection": "0",
                "ewaldTolerance": ".0005",
                "exceptionsUsePeriodic": "0",
                "includeDirectSpace": "1",
                "ljAlpha": "0",
                "ljnx": "0",
                "ljny": "0",
                "ljnz": "0",
        ...
        for i in range(self.ff.n_atoms):
            for j in range(i + 1, self.ff.n_atoms):
                close_neighbor = any(
                    [
                        j in self.ff.topo.neighbors[c][i]
                        for c in range(self.ff.non_bonded.n_excl)
                    ]
                )
                if close_neighbor or (i, j) in self.ff.non_bonded.exclusions:
                    ET.SubElement(
                        exceptions,
                        "Exception",
                        {"p1": str(i), "p2": str(j), "q": "0", "eps": "0", "sig": "0"},
                    )
    ...
    def write_lennard_jones(self, forces):
        ...
        for lj_type in self.ff.non_bonded.lj_types:
            ...
            ET.SubElement(
                particles, "Particle", {"param1": str(param1), "param2": str(param2)}
            )
        ...
        for i in range(self.ff.n_atoms):
            for j in range(i + 1, self.ff.n_atoms):
                ...
                if (
                    close_neighbor
                    or (i, j) in self.ff.non_bonded.exclusions
                    or (i, j) in self.ff.non_bonded.pairs
                ):
    ...
    def write_lennard_jones_14(self, forces):
        ...
        for i, j in self.ff.non_bonded.pairs:
            pair_name = tuple(
                sorted([self.ff.non_bonded.lj_types[i], self.ff.non_bonded.lj_types[j]])
            )
            ...
            if pair_name in self.ff.non_bonded.lj_1_4.keys():
                ...
                param1, param2 = [
                    p * self.ff.non_bonded.fudge_lj
                    for p in self.ff.non_bonded.lj_pairs[pair_name]
                ]
