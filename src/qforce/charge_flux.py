...
def fit_charge_flux(qm_out, qm_energy_out, qm_gradient_out, mol, weight_dipole=0.1):
    differences, cf_contrib = calc_dipole_derivative(
        qm_out.dipole_deriv, qm_out.coords.copy(), mol.non_bonded.q, mol
    )

    if len(qm_energy_out + qm_gradient_out) > 0:
        frame_diff, frame_cf_contrib = calc_additional_frames(
            qm_energy_out, qm_gradient_out, mol, weight_dipole
        )
...
def calc_additional_frames(qm_energy_out, qm_gradient_out, mol, weight_dipole):
    ...
    for out in qm_energy_out + qm_gradient_out:
        ...
        mm_dip_cf = np.array(
            [calc_dipole_from_charge(out.coords, q) for q in dq_mm_dip.T]
        )
...
def calc_dipole_derivative(qm_dipole_deriv, coords, charge, mol, disp=1e-5):
    ...
    dq_dipole_derivative = np.zeros(
        (mol.topo.n_atoms, 3, 3, mol.terms.n_fitted_flux_terms)
    )
    ...
    dq_dipole_derivative = dq_dipole_derivative.reshape(
        (-1, mol.terms.n_fitted_flux_terms)
    )
