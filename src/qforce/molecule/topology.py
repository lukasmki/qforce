...
class Topology(object):
    ...
    def __init__(self, config, mol):
        ...
        #
        self.neighbors = [
            [[] for j in range(self.n_atoms)] for i in range(3)
        ]  # First 3 neighbors
        ...
        self.unique_atomids = []  #
        self.atoms = np.zeros(
            self.n_atoms, dtype="int8"
        )  # unique atom numbers of each atom
        ...
        for i_idx, i_elem in enumerate(self.atomids):
            ...
            if self.node(i_idx)["n_neighs"] == 1:
                ...
                theta = get_angle(
                    self.coords[
                        [
                            self.node(i_idx)["neighs"][0],
                            i_idx,
                            self.node(i_idx)["neighs"][1],
                        ]
                    ]
                )[0]
    ...
    def _find_atom_types(self):
        ...
        for i in range(self.n_atoms):
            ...
            self.node(i)["neighs"] = sorted(
                self.node(i)["neighs"],
                reverse=True,
                key=lambda e: (
                    self.node(e)["n_neighs"],
                    self.node(e)["elem"],
                    self.node(e)["idx"],
                ),
            )
            self.node(i)["nonrepeat_neighs"] = sorted(
                self.node(i)["nonrepeat_neighs"],
                reverse=True,
                key=lambda e: (
                    self.node(e)["n_neighs"],
                    self.node(e)["elem"],
