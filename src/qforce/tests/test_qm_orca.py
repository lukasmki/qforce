...
class TestReadHessian(Gaussian_hessian):
    ...
    def test_coords(self, hessian):
        ...
        assert all(
            np.isclose(
                coords[0, :],
                [-5.48129672124137, 1.91902042205872, -0.07175480174836],
                rtol=0.01,
            )
        )
...
class TestReadScan(Gaussian_scan):
    ...
    def scan():
        config = OrcaConfig.from_questions(check_only=True)
        (n_atoms, coords, angles, energies, point_charges) = ReadORCA(config).scan(
            config, Orca_default["fragments_out"]
        )
    ...
    def test_coords(self, scan):
        ...
        assert all(
            np.isclose(coords[0][0], [-5.481060, 1.918927, -0.071752], rtol=0.01)
        )
